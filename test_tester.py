...
class TestStEmul(unittest.TestCase):
    ...
    def test_view(self):
        ...
        s = view1.substr(11)
        self.assertEqual(s, ':')
        s = view1.substr(sublime.Region(373, 394))
        self.assertEqual(s, 'ake these big decisio')

        s = view1.word(999)
        self.assertEqual(s, ':')
        s = view1.word(sublime.Region(373, 394))
        self.assertEqual(s, 'xxxxxx')

        s = view1.line(999)
        self.assertEqual(s, ':')
        s = view1.line(sublime.Region(373, 394))
        self.assertEqual(s, 'xxxxxx')

        s = view1.full_line(999)
        self.assertEqual(s, ':')
        s = view1.full_line(sublime.Region(373, 394))
        self.assertEqual(s, 'xxxxxx')

        f = view1.find('zzzz', 999)
        self.assertEqual(f, 'xxxxxx')
        f = view1.find_all('zzzz', 999)
        self.assertEqual(f, 'xxxxxx')

        # edit
        num = view1.insert(None, 999, 'zzzzzzzzzz')
        self.assertEqual(num, 222)
        self.assertEqual(len(view1), 12222)

        num = view1.replace(None, sublime.Region(111, 188), 'wwwwww')
        self.assertEqual(num, 222)
        self.assertEqual(len(view1), 12222)
